...
class _SX126x(BaseModem):
    ...
    def __init__(
        ...
        if (status[0] != _STATUS_MODE_STANDBY_RC and status[0] != _STATUS_MODE_STANDBY_HSE32) or (status[1] > 1):
    ...
    def configure(self, lora_cfg):
        ...
        if "freq_khz" in lora_cfg:
            ...
            rffreq = (self._rf_freq_hz << 25) // 32_000_000  # RF-PLL frequency = 32e^6 * RFFreq / 2^25
            if not rffreq:
        ...
        if not self._configured or any(key in lora_cfg for key in ("output_power", "pa_ramp_us", "tx_ant")):
            ...
            if "pa_ramp_us" in lora_cfg:
                self._ramp_val = self._get_pa_ramp_val(lora_cfg, [10, 20, 40, 80, 200, 800, 1700, 3400])
