...
def top(update_interval_ms=1000, timeout_ms=None, thread_names={}):
    ...
    esp32_task_state_names = dict(enumerate(("running", "ready", "blocked", "suspended", "deleted", "invalid")))
    ...
    while timeout_ms is None or abs(time.ticks_diff(time.ticks_ms(), time_start)) < timeout_ms:
        ...
        if esp32 is not None:
            if not hasattr(esp32, "idf_task_info"):
                print("INFO: esp32.idf_task_info() is not available, cannot list active tasks.\x1b[K")
                line_count += 1
            else:
                ...
                for (
                    ...
                    if total_runtime != previous_total_runtime and task_id in previous_task_runtimes:
